...
async function unlock() {
  ...
  try {
    const res = await fetch("musiccrypt.txt");
...
async function upload() {
  ...
  const proceed = async () => {
    ...
    try {
      const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/Music.txt`, {
      ...
      await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/Music.txt`, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
          message: `Update Music.txt with new entry`,
